    ...
    try:
        ...
        class DuelingDQN(nn.Module):
            ...
            def forward(self, state):
                # ✅ Verify state is correct shape: (batch, 45)
                if state.shape[-1] != 45:
...
def encode_user_input(user_data, artifacts):
    ...
    ✅ CRITICAL FIX: Ensure exactly 45 dimensions
    """
    
    state = []
    
    # CATEGORICAL FEATURES (convert to 0-1)
    ...
    # Encode 11 categorical features
    ...
    for feature in categorical_features:
        ...
        if feature in categorical_mappings:
            ...
            encoded = mapping.get(value, 0.5)  # Default to 0.5 if unknown
        else:
            encoded = 0.5
        state.append(encoded)
    
    # NUMERICAL FEATURES (normalize to 0-1)
    # Get scaler parameters from artifacts
    scaler_min = artifacts['scaler_params']['min']
    scaler_max = artifacts['scaler_params']['max']
    ...
    for i, feature in enumerate(numerical_features):
        value = float(user_data.get(feature, 0.0))
        
        # Normalize using scaler parameters
        if i < len(scaler_min):
            min_val = scaler_min[i]
            max_val = scaler_max[i]
            ...
            if value > 100:
                ...
                normalized = value / max(value, 1.0)
        
        state.append(normalized)
    
    # ✅ ENSURE EXACTLY 45 dimensions
    state = state[:45]  # Truncate if too long
    while len(state) < 45:  # Pad if too short
        state.append(0.0)
...
def predict_career_path_diverse(user_data, num_paths=3):
    """Generate diverse career recommendations"""
    ...
    for path_num in range(num_paths):
        ...
        initial_options = []
        for career_id, career_name in resources['id_to_career'].items():
            if stream in ['Science-PCM', 'Science-PCMB']:
                if any(x in career_name for x in ['Computer', 'Data', 'Engineering', 'AI', 'ML']):
                    initial_options.append(career_name)
            elif stream == 'Science-PCB':
                if any(x in career_name for x in ['MBBS', 'BDS', 'B.Pharm', 'Nursing']):
                    initial_options.append(career_name)
            elif stream == 'Commerce':
                if any(x in career_name for x in ['CA', 'BBA', 'B.Com', 'Finance']):
                    initial_options.append(career_name)
            elif stream == 'Arts':
                if any(x in career_name for x in ['Law', 'Psychology', 'Communication']):
                    initial_options.append(career_name)
        
        if not initial_options:
            initial_options = list(resources['id_to_career'].values())[:5]
        
        current_career = np.random.choice(initial_options)
        path = [current_career]
        total_reward = 0
        
        # Temperature increases for exploration
        temperature = 1.0 + (path_num * 0.5)
        
        # ✅ CRITICAL FIX: Encode state properly (45 dimensions)
        state = encode_user_input(user_data, resources['artifacts'])
        
        for step in range(4):
            # Get valid careers
            if current_career in resources['graph']:
                successors = list(resources['graph'].successors(current_career))
                valid_careers = successors if successors else [current_career]
            else:
                valid_careers = list(resources['id_to_career'].values())[:10]
            
            valid_ids = []
            for career in valid_careers:
                if career in resources['artifacts']['career_to_id']:
                    valid_ids.append(resources['artifacts']['career_to_id'][career])
            
            if not valid_ids:
                valid_ids = list(range(len(resources['id_to_career'])))
            
            with torch.no_grad():
                # ✅ CRITICAL FIX: Ensure state is 45-dimensional
                state_check = state if len(state) == 45 else np.pad(state, (0, 45-len(state)))
                state_tensor = torch.FloatTensor(state_check).unsqueeze(0).to(resources['device'])
                q_values = resources['model'](state_tensor).squeeze()
                
                # Get Q-values for valid actions
                valid_q = torch.tensor([q_values[idx].item() if idx < len(q_values) else 0.0 
                                       for idx in valid_ids], device=resources['device'])
                ...
                for i, action_id in enumerate(valid_ids):
                    next_career = resources['id_to_career'][action_id]
            ...
            next_career = resources['id_to_career'][best_action]
            path.append(next_career)
            current_career = next_career
        
        path_info = {
            'path': path,
            'score': total_reward / 5.0 if len(path) > 0 else 0,
            'length': len(path)
        }
